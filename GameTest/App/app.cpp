...
{	
	...
	void DrawNgon(float cx, float cy, float radius, float startAngle, float endAngle, float r, float g, float b, int sliceCount)
	{
		float virtualX = cx;
		float virtualY = cy;
		...
		glColor3f(r, g, b);
#if APP_USE_VIRTUAL_RES		
		APP_VIRTUAL_TO_NATIVE_COORDS(virtualX, virtualY);
#endif
		glVertex2f(virtualX, virtualY);
		for (int i = startAngle; i <= endAngle; i += 360 / sliceCount)
		{
			float xCir = cx + radius * cosf(i * PI / 180.0f);
			float yCir = cy + radius * sinf(i * PI / 180.0f);
#if APP_USE_VIRTUAL_RES		
			APP_VIRTUAL_TO_NATIVE_COORDS(xCir, yCir);
#endif
			glVertex2f(xCir, yCir);
		}
		glEnd();
	}
